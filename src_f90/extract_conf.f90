...
SUBROUTINE COMPUTE_COM_DIST(iframe)
  ...
  IF(multiposflag) THEN
     ...
     IF(remdval .LE. tol)THEN
        ...
        CALL EXTRACT_AND_WRITE(absdval)
        genflag = 1
  ...
  IF(comflag) THEN
     ...
     WRITE(comout,'(I0,1X,6(F14.8,1X))') iframe,comgrp(1,1),comgrp(1&
          &,2),comgrp(1,3),comgrp(2,1),comgrp(2,2),comgrp(2,3)
...
SUBROUTINE EXTRACT_AND_WRITE(dum_dval)
...
2000 FORMAT(2(F15.8,1X),2X,A)
  ...
  DO i = 1,ntotatoms
     ...
     WRITE(outfile,'(3(I0,1X),4(F15.9,1X))') aidvals(aid,1)&
          &,aidvals(aid,2),aidvals(aid,3),charge_lmp(aid,1)&
          &,rxyz_lmp(aid,1),rxyz_lmp(aid,2),rxyz_lmp(aid,3)
